...
//const client = redis.createClient(6379, 'place-001.tvsqkd.0001.cac1.cache.amazonaws.com'); //connecting to our redis server
	...
	ws.on('message', function(message) {
		...
		if(isValidSet(clientMessage)) {
			...
			myPromise.then((data) => {
				...
				if(data.Count == 0 || currentDate - (new Date(data.Items[0]["timestamp"])) >= 30000) {
				...
				console.log(data);
				//client.bitfield("board","set", "u8", String(4*x + 1000*y), o.colour);
